...
- Control flow: `fail` raises a private abort caught in exactly two places — `attempt`, the parser's only speculation primitive (fork + contained abort = tentative parse with rollback, cf. Clang's `TentativeParsingAction`), and `parser.parse`, the fail-fast catch (the Lua lparser longjmp idiom; error path only, never speculation). A grammar spot that seems to need backtracking routes through `attempt` or restructures with lookahead; a `pcall` anywhere else in the parser is a design smell.
